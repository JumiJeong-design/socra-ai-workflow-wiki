...
이번 주에는 Prism 디자인 시스템을 Figma 중심으로 다시 정렬하고, Git 문서와 Storybook 검수 환경이 Figma를 따라오도록 Button, Tooltip, Typography, 운영 워크플로우를 함께 정비했다.

## 이번 주에 디벨롭한 핵심 내용

### 1. Figma-first / Storybook-verified 운영 기준 정립

- Figma를 시각 판단과 디자인 탐색의 source of truth로 두고, Git 문서와 Storybook은 Figma에서 확정된 값을 따라가도록 기준을 정리했다.
- Figma보다 Git이나 Storybook이 먼저 앞서가면 sync가 깨진다는 문제를 확인했고, 이를 방지하기 위해 agent rules와 ADR에 Figma source first 원칙을 추가했다.
- Storybook은 예쁜 문서 페이지가 아니라, 확정된 컴포넌트 상태와 조합을 검수하는 표면으로 역할을 재정의했다.

공유 포인트:
- 앞으로 컴포넌트 수정은 Figma 원본 확인 -> Git spec 반영 -> Storybook 검수 순서로 진행한다.
- Storybook에서 보이는 화면은 최종 조합 검수 기준으로 사용하되, 신규 디자인 탐색은 Figma에서 먼저 진행한다.
...
### Agenda 1. Prism 디자인 시스템의 source of truth 운영 방식 합의

논의 질문:
- 컴포넌트 수정 요청은 Figma에서 먼저 확정하고 Storybook에서 검수하는 흐름으로 고정할지?
- Figma component, Git spec, Storybook story 중 어디까지를 디자이너가 직접 관리할지?
- Code Connect는 지금 당장 필요한지, 아니면 Figma node id + Git spec + Storybook mapping으로 먼저 운영할지?

결정이 필요한 것:
- Figma-first / Storybook-verified 운영 모델을 팀 기준으로 채택할지
...
이번 주에는 Prism 디자인 시스템을 Figma 기준으로 다시 정렬하는 작업을 했습니다. 처음에는 Git 문서나 Storybook이 Figma보다 앞서가면서 sync가 깨질 수 있는 부분이 있었고, 그래서 Figma-first / Storybook-verified 원칙을 명확히 잡았습니다.
...
- AI Workflow Guide v0.11: https://jumijeong-design.github.io/socra-ai-workflow-guide/ai-workflow-guide.html
- Changelog v0.11: https://jumijeong-design.github.io/socra-ai-workflow-guide/changelog.html
