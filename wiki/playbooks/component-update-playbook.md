...
| `approved component` | `socraAI_product design`의 최신 contract와 Storybook 기준을 확인한다. |
...
6. 현재 구현 계약 변경이 필요하면 `socraAI_product design`의 최신 AGENTS/README와 관련 contract 문서를 먼저 확인한다.
7. 실제 component/token/package contract 수정은 product-design repo에서 수행한다.
8. 이 wiki에는 반복 가능한 판단 기준, 실수 방지 규칙, handoff 요약만 남긴다.
9. 구현이 있다면 product-design repo의 검증 기준에 따라 Storybook 또는 preview에서 확인한다.
10. PR/기록에는 Figma link, product-design source-of-truth 문서, worklog/wiki lesson 중 무엇을 연결해야 하는지 구분한다.

## Done Criteria

- Figma source status가 명확하다.
- Exploration/Candidate 작업이 실수로 implementation contract가 되지 않았다.
- 현재 구현 계약의 source-of-truth가 product-design repo인지 확인했다.
- product-design contract를 이 wiki에 중복 작성하지 않았다.
- 구현된 visual value 검증은 product-design repo의 기준을 따른다.
