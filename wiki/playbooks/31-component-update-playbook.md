...
Figma 컴포넌트, `riiid/prism` 계약, 코드 컴포넌트, Storybook 스토리 중 하나가 바뀌어 어느 저장소가 판단을 소유하는지 정리해야 할 때 사용한다.
...
| 상태 | 처리 방법 |
| --- | --- |
| `탐색(exploration)` | 워크로그/맥락만 남기고 `riiid/prism` 계약/코드/Storybook은 바꾸지 않는다. |
...
3. 승인된 컴포넌트가 아니면 `riiid/prism` 계약으로 승격하지 않는다.
...
토큰이나 공개 CSS 변수 변경은 source 수정과 생성물 반영을 분리해서 확인한다.

| 층 | 확인할 것 |
| --- | --- |
| Source | `*.tokens.json`, 컴포넌트 코드, contract 문서가 의도대로 바뀌었는가 |
| Generated | `theme.css`, `public.manifest.json`, release manifest 같은 산출물이 단일 owner에 의해 갱신됐는가 |
...
`riiid/prism`처럼 디자이너와 프론트 개발자가 같은 패키지를 함께 수정할 때의 합의다 (2026-08 프론트 개발자 싱크).

- **수정 주체는 양쪽 다.** 충돌을 미리 막는 강제 장치(CODEOWNERS 등)는 불편해서 지양하고, 실제 충돌이 나는지 관찰하며 가드레일을 보완한다. 충돌 가능성을 모두 인지하고, 났을 때 에이전트가 잘 해결하는지 확인한다.
- **문서를 독자 기준으로 나눈다.** 패키지 안 문서는 대부분 에이전트 대상이라 사람이 다 읽기 어렵다. 사람이 읽는 문서(디자인 문서 등)와 에이전트 대상 문서를 폴더 단위로 구분한다.
- **건드리면 안 되는 경로를 목록으로 전달한다.** 디자이너가 적극 관리하는 핵심 문서 목록을 개발자에게 공유하고, 그 외는 개발자가 자유롭게 수정한다.
- **기능 로직이 디자인 컴포넌트에 들어가면 개발자가 가드레일을 추가한다.** 이미 들어가 있는 컴포넌트도 정리 대상이다.
...
- `riiid/prism` 계약을 이 wiki에 중복 작성하지 않았다.
...
- Notion 「PRISM 패키지 싱크」 (2026-08, 프론트 개발자 합의)
